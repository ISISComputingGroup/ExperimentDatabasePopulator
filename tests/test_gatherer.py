...
class GathererTests(unittest.TestCase):
    ...
    def test_GIVEN_instrument_list_has_scheduled_instrument_WHEN_gatherer_started_THEN_update_runs(
        self, gather_data, update
    ):
    ...
    @patch("exp_db_populator.gatherer.update")
    @patch("exp_db_populator.gatherer.gather_data")
    def test_GIVEN_instrument_list_has_unscheduled_instrument_WHEN_gatherer_started_THEN_no_update(
        self, gather_data, update
    ):
        new_name, new_host = "TEST", "NDXTEST"
